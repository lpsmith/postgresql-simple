...
liftPQ :: (HasCallStack) => B.ByteString -> Connection -> (PQ.Connection -> IO (Maybe a)) -> IO a
...
loCreat :: (HasCallStack) => Connection -> IO Oid
loCreat conn = liftPQ "loCreat" conn (\c -> PQ.loCreat c)

loCreate :: (HasCallStack) => Connection -> Oid -> IO Oid
loCreate conn oid = liftPQ "loCreate" conn (\c -> PQ.loCreate c oid)

loImport :: (HasCallStack) => Connection -> FilePath -> IO Oid
loImport conn path = liftPQ "loImport" conn (\c -> PQ.loImport c path)

loImportWithOid :: (HasCallStack) => Connection -> FilePath -> Oid -> IO Oid
